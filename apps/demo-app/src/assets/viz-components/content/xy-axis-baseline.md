...
By default / if the baseline method is never called, baselines are drawn on quantitative axes and
omitted on ordinal axes. If there are positive and negative values on a chart, the baseline for the
perpendicuar axis will be default be repositioned to the zero tick mark.
...
**Example of configuration for `baseline`**

```ts
// x-axis baseline that is solid when there are positive and negative values
this.xOrdinalAxis
  .baseline((baseline) => baseline.display(true).zeroBaseline({ dasharray: 'none' }))
  .getConfig();
```
...
params:
  - name: zeroBaseline
    ...
    description:
      ...
      - 'If only one of these properties is specified, the other will default to its default value.'
