import { VicOrdinalDimension, vicOrdinalDimension } from './ordinal-dimension';
...
describe('VicOrdinalDimension', () => {
  ...
  describe('setDomain', () => {
    it('sets the domain to the correct value, user did not specify domain', () => {
      dimension.setPropertiesFromData(data, false);
      expect([...(dimension as any).internSetDomain.values()]).toEqual([
        'a',
        'b',
        'c',
      ]);
