...
export function positionAtCentroid<P, G extends Geometry>(
...
export function positionHawaiiOnGeoAlbersUsa<
  P,
...
>(feature: GeographiesFeature<P, G>, projection: Projection): LabelPosition {
  const startPolygon =
    ...
    (feature.geometry.coordinates[0][0] as [number, number][]).map(projection);
  const endPolygon = (
...
export function positionWithPolylabel<
  P,
